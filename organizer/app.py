...
class OrganizerApp(tk.Tk):
    ...
    def _build_widgets(self) -> None:
        ...
        ttk.Label(config, text="低于此值会进入“无法分类”", style="Muted.TLabel").grid(row=0, column=4, padx=(12, 0), sticky="w")
        ...
        api_key_entry.grid(row=1, column=1, columnspan=4, pady=(10, 0), sticky="ew")
        ToolTip(api_key_entry, "仅用于调用 DeepSeek；会优先保存在 Windows 凭据管理器，不写入普通设置文件。")
        ttk.Label(config, text="首次使用填写一次；安全保存在 Windows 凭据管理器。", style="Muted.TLabel").grid(row=2, column=1, columnspan=4, pady=(3, 0), sticky="w")
